...
def administrator():
    ...
    if request.method == 'POST':
        ...
        password = userDetails['password']
        cur = mysql.connection.cursor()
        okay=cur.execute("Select * from Admin where  (name=%s and password=%s)",(name,password))
        if okay>0:
            userid = cur.fetchall()
            global current_user_id 
            current_user_id = userid[0][0]
...
def IT_MEMBERS():
    if request.method == 'GET':
        cursor = mysql.connection.cursor()
        things=cursor.execute("Select * from Admin")
...
def receive():
    if request.method == 'POST':
        ...
        Equipments = userDetails['Equipments']
        cursor = mysql.connection.cursor()
        cursor.execute('DELETE FROM issued WHERE name=%s and Equipment=%s', (name,Equipments))
        mysql.connection.commit()
        return redirect("/design")
...
def adduser():
    if request.method == 'POST':
        ...
        if confirmpass == password:
            cur = mysql.connection.cursor()
            cur.execute("INSERT INTO Admin (name, password) VALUES(%s,%s)",(name, password))
            mysql.connection.commit()
            cur.close()
            return redirect("/design")
